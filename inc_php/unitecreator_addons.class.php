...
class UniteCreatorAddons extends UniteElementsBaseUC{
	 ...
	 */
	protected function getCatsAndAddonsHtml($catID, $type, $data = null){
		
		$objManager = UniteCreatorManager::getObjManagerByAddonType($type, $data);
				
		$response = $objManager->getCatsAndAddonsHtml($catID);
	...
	public function removeAddonsFromData($data){
		...
		$response = $this->getCatsAndAddonsHtml($catID, $type, $data);
	...
	public function moveAddonsFromData($data){
		...
		if($isLayouts == false)
			...
			$objLayouts = new UniteCreatorLayouts();
			$objLayouts->moveLayouts($arrAddonIDs, $targetCatID);
		}
		
			
		$repsonse = $this->getCatsAndAddonsHtml($selectedCatID, $type, $data);
		return($repsonse);
	}
