...
class UniteCreatorElementorIntegrate{
	...
	const ADDONS_CATEGORY_NAME = "unlimited_elementor_elements";
	...
	public static function getCurrentTemplateType(){
		...
		if($postType == Globalselementwidgetpro::POSTTYPE_ELEMENTOR_LIBRARY){
			
			$templateType = get_post_meta($post->ID, Globalselementwidgetpro::META_TEMPLATE_TYPE, true);
			return($templateType);		
		}
	...
	private function initTemplateTypeVars(){
		...
		EWPHelper::addDebug("set template type: ".self::$templateType);
	}
	...
	public function onSectionStyleControlsAdd($objControls, $args){
		...
		$arrBGItems = array();
		$arrBGItems[$none] = __("[Not Selected]", "unlimited_elementor_elements");
        ...
        $objControls->add_control(
              'title', array(
              'label' => __("Background Type", "unlimited_elementor_elements"),
    ...
    private function isElementorLibraryPage(){
		...
		if($current_screen->post_type != Globalselementwidgetpro::POSTTYPE_ELEMENTOR_LIBRARY)
			return(false);
	...
	*/
	...
	private function putImportLayoutButton(){
		...
		<div style="display:none">
			...
			<a id="uc_button_import_layout" href="javascript:void(0)" class="page-title-action"><?php esc_html_e("Import Template With Addons", "unlimited_elementor_elements")?></a>
			...
			<div id="uc_import_layout_area" style="display:none">
				...
				<form id="uc_form_import_template" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					...
					<fieldset>
						...
						<input type="submit" class="button" value="<?php esc_html_e( 'Import Now', "unlimited_elementor_elements" ); ?>">
					</fieldset>
				</form>
			</div>
    ...
    public function onAddScripts(){
    	...
    	if($isTemplatesPage == true)
    		EWPHelper::addScriptAbsoluteUrl(HelperProviderCoreUC_EL::$urlCore."elementor/assets/template_library_admin.js", "unlimited_addons_template_library_admin");
    }
    ...
    public function onRegisterFrontScripts(){
    	...
    	EWPHelper::addScriptAbsoluteUrl(HelperProviderCoreUC_EL::$urlCore."elementor/assets/uc_front_admin.js", "unlimited_elments_editor_admin");
    ...
    public function onEnqueueEditorScripts(){
    	...
    	EWPHelper::addScriptAbsoluteUrl(HelperProviderCoreUC_EL::$urlCore."elementor/assets/uc_editor_admin.js", "unlimited_elments_editor_admin");
    	EWPHelper::addStyleAbsoluteUrl(HelperProviderCoreUC_EL::$urlCore."elementor/assets/uc_editor_admin.css", "unlimited_elments_editor_admin_css");
    ...
    public function onAjaxImportLayout(){
    	...
    	try{
	    	...
	    	wp_redirect(Globalselementwidgetpro::$urlTemplatesList);
	    	exit();
    ...
    public static function logMemoryUsage($operation, $updateOption=false){
    	...
    	EWPHelper::logMemoryUsage($operation, $updateOption);
    	
    	$isEnoughtMemory = UniteFunctionsUC::isEnoughtPHPMemory();
    	if($isEnoughtMemory == false)
    		EWPHelper::logMemoryUsage("Low Memory!!!", true);
	...
	private function isTemplatesScreen(){
		...
		if($current_screen->base == "edit" && $current_screen->post_type == Globalselementwidgetpro::POSTTYPE_ELEMENTOR_LIBRARY)
			return(true);
	...
	public function postRowActions($actions, WP_Post $post ){
		...
		$actions['export-template-withaddons'] = sprintf( '<a href="%1$s">%2$s</a>', $this->getTemplateExportWithAddonsLink($postID), __( 'Export With Addons', "unlimited_elementor_elements" ) );
