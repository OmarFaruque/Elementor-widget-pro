...
class UniteCreatorElementorWidget extends Widget_Base {
	        ...
	        $arrFontsParams = $this->objAddon->getArrFontsParams();
          	...
          	foreach($arrFontsSections as $name=>$title){
          		...
          		$this->start_controls_section(
		                ...
		                'label' => $title." ".esc_html__("Styles", "unlimited_elementor_elements"),
          				'tab' => "style"
		                 )
		         );
    ...
    protected function addItemsControlsUC($itemsType){
    	...
    	if($itemsType == "image")
         ...
         $this->start_controls_section(
                'section_items', array(
                'label' => esc_html__("Items", "unlimited_elementor_elements"),
                    )
          );
    ...
    protected function addTypographyByParamUC($param){
    	...
    	$controlName = EWPHelper::convertTitleToHandle($name);
    ...
    private function addEditAddonControl(){
    	...
    	$urlEditAddon = EWPHelper::getViewUrl_EditAddon($addonID, "", "tab=uc_tablink_html");
   ...
   protected function ucRegisterControls_addon(){
         ...
         $this->start_controls_section(
                ...
                'label' => esc_html__("General", "unlimited_elementor_elements"),
                    )
          );
          ...
          if($hasPostsList == true){
	          ...
	          $this->start_controls_section(
	                'section_query', array(
	                'label' => esc_html__("Query", "unlimited_elementor_elements"),
	                    )
	          );
          ...
          if($hasPostsList == true && ($isArchiveLocation == true || UniteCreatorElementorIntegrate::$isAjaxAction == true))
   ...
   protected function ucRegisterControls_cat_addAddonsDropdown($meta){
   		...
   		$control["label"] = esc_html__("Select Style", "unlimited_elementor_elements");
    ...
    */
    ...
    protected function ucRenderByAddon($objAddon){
    	try{
	    	...
	    	EWPHelper::addDebug("widget values", $arrValues);
    ...
    protected function render() {
    	...
    	if($this->isConsolidated == false){
    		...
    		try{
    			...
    			echo "error render addon: $addonName";
    			HelperHtmlUC::outputException($e);
    		}
